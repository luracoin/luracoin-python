...
class BlocksTest(LuracoinTest):
    ...
    def test_serialize_block(self) -> None:
        signature = bytes_to_signing_key(self.private_key2).sign(
            build_message(OutPoint(self.tx2.id, 0), self.public_key2).encode()
        )
        ...
        tx1 = Transaction(
            ...
            txouts=[
                TxOut(
                    value=1_000_000_000, to_address=build_p2pkh(self.address2)
                ),
                TxOut(
                    value=2_000_000_000, to_address=build_p2pkh(self.address3)
                ),
                TxOut(
                    value=2_000_000_000, to_address=build_p2pkh(self.address3)
                ),
        ...
        self.assertEqual(
            block1_new.txns[0].txouts[0].value, block1.txns[0].txouts[0].value
        )
        self.assertEqual(
        ...
        self.assertEqual(
            block1_new.txns[1].txouts[0].value, block1.txns[1].txouts[0].value
        )
        self.assertEqual(
