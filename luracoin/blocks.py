...
from .helpers import get_blk_file_size, var_int
...
def validate_block(block):
    ...
    if (
        validate_pow(block)
        and validate_basics(block)
        and validate_transactions(block)
    ):
        return True
...
def add_block_to_chain(serialized_block):
    ...
    if validate_block(serialized_block):
        ...
        size_block = (
            int(len(serialized_block) - 8)
            .to_bytes(4, byteorder="little", signed=False)
            .hex()
        )
        serialized_block = (
            serialized_block[:8] + size_block + serialized_block[8:]
        )
